...
<body>
    ...
    <div class="container col-12 col-md-12 col-lg-6 mt-3">
        <div class="row">
            <div class="col">
                <h1 class="text-primary">FAED's Link</h1>
            </div>
            <div class="col">
                <div class="float-end">
        ...
        if (isset($_GET["a"]) && $_GET["a"] == "createnew" && $_POST["fst"] == "createnew") {
            ...
            $data_form = array("code" => $code, "title" => "คณะสถาปัตย์ฯ", "url" => $_POST["url"], "user" => $_SESSION["admin"]["fistNameEn"], "citizenId" => $_SESSION["admin"]["citizenId"], "time" => time(), "status" => "enable");
            // print_r($data_form);
            // echo "your link : " . $fnc->url_hosting . $code . "<br>";

            $str_data = $fnc->fread_data($_SESSION["admin"]["fistNameEn"]);
            if (!is_null($str_data)) {
                $data_array = array();
            ...
            $fnc->fwrite_data($data,$_SESSION["admin"]["fistNameEn"]);
        ...
        // if (isset($_GET["a"]) && $_GET["a"] == "fread") {
            $data = json_decode($fnc->fread_data($_SESSION["admin"]["fistNameEn"]), true, JSON_UNESCAPED_UNICODE);
            if (is_array($data)) {
                // print_r($data);
                $i = 0;
                ...
                foreach ($data as $d) {
                    ...
                    if ($d["status"] == "enable" && $d["citizenId"] == $_SESSION["admin"]["citizenId"]) {
